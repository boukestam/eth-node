// Modified from: https://github.com/ethereumjs/ethereumjs-devp2p/blob/master/src/rlpx/ecies.ts
...
export class ECIES {
  ...
  _gotEIP8Auth: boolean = true
  _gotEIP8Ack: boolean = true
  ...
  constructor(privateKey: Buffer, id: Buffer, remoteId: Buffer) {
    ...
    this._publicKey = idToPK(id)
    this._remotePublicKey = remoteId ? idToPK(remoteId) : null

    this._nonce = crypto.randomBytes(32)
    this._ephemeralPrivateKey = generatePrivateKey()
  ...
  createAuthEIP8() {
    ...
    const data = [
      ...
      // keccak256(pkToId(this._ephemeralPublicKey)),
      pkToId(this._publicKey),
      this._nonce,
      Buffer.from([0x05])
    ]
    ...
    const sharedMacData = intToBuffer(authMsg.length + overheadLength)
  ...
  parseAuthPlain(data: Buffer, sharedMacData: Buffer | null = null): Buffer | undefined {
    ...
    if (!this._gotEIP8Auth) {
      ...
      remotePublicKey = idToPK(decrypted.slice(97, 161))
      ...
      remotePublicKey = idToPK(decoded[1])
      nonce = decoded[2]
    }
    ...
    if (heid !== null && this._remoteEphemeralPublicKey) {
      assertEq(
        keccak256(pkToId(this._remoteEphemeralPublicKey)),
        heid,
  ...
  parseAuthEIP8(data: Buffer): void {
    const size = bufferToInt(data.slice(0, 2)) + 2
  ...
  createAckEIP8(): Buffer | undefined {
    ...
    const overheadLength = 113
    const sharedMacData = intToBuffer(ackMsg.length + overheadLength)
  ...
  createAckOld(): Buffer | undefined {
    const data = Buffer.concat([pkToId(this._ephemeralPublicKey), this._nonce, Buffer.from([0x00])])
  ...
  parseAckPlain(data: Buffer, sharedMacData: Buffer | null = null): void {
    ...
    if (!this._gotEIP8Ack) {
      ...
      remoteEphemeralPublicKey = idToPK(decrypted.slice(0, 64))
      remoteNonce = decrypted.slice(64, 96)
    } else {
      const decoded = unstrictDecode(decrypted) as Buffer[];

      remoteEphemeralPublicKey = idToPK(decoded[0])
